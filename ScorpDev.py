...
def create_react_project(folder):
    ...
    subprocess.Popen(["npm", "start"], cwd=f"{folder}/react-client", shell=True)

def create_flask_project(folder):
    print("Creating Flask project...")
    flask_server_path = f"{folder}/flask-server"
    try:
        subprocess.run(["mkdir", flask_server_path], check=True, shell=True)
        subprocess.run(["python", "-m", "venv", f"{flask_server_path}/venv"], check=True, shell=True)
        subprocess.run([f"{flask_server_path}/venv/Scripts/pip", "install", "Flask"], check=True, shell=True)
        with open(f"{flask_server_path}/app.py", "w") as file:
...
if __name__ == '__main__':
    app.run(debug=True)''')
        subprocess.Popen([f"{flask_server_path}/venv/Scripts/flask", "run"], cwd=flask_server_path, shell=True)
    except subprocess.CalledProcessError as e:
